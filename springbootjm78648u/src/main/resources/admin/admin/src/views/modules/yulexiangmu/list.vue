<template>
	<div class="main-content" :style='{"width":"calc(100% - 40px)","padding":"20px 30px","margin":"20px 0 0 auto","fontSize":"15px","borderRadius":"10px","background":"#fff"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","flexWrap":"wrap","background":"#fff","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">娱乐名称</label>
						<el-input v-model="searchForm.yulemingcheng" placeholder="娱乐名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yulexiangmu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yulexiangmu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu1" :style='{"margin":"0 2px","fontSize":"14px","color":"#107ED6","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yulexiangmu','参与人数统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yulexiangmu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yulexiangmu','立即参与')" @click="canyuxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已参与')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								立即参与
							</el-button>
							<el-button class="edit" v-if=" isAuth('yulexiangmu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('yulexiangmu','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yulexiangmu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<canyuxinxi-cross-add-or-update v-if="canyuxinxiCrossAddOrUpdateFlag" :parent="this" ref="canyuxinxiCrossaddOrUpdate"></canyuxinxi-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="canyurenshuChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import canyuxinxiCrossAddOrUpdate from "../canyuxinxi/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				canyuxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			canyuxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.canyuxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yulexiangmu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.canyuxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.canyuxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var canyurenshuChart1 = echarts.init(document.getElementById("canyurenshuChart1"),'macarons');
					this.$http({
						url: `yulexiangmu/value/yulemingcheng/canyurenshu`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].yulemingcheng);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].yulemingcheng
								})
							}
							var option = {};
							let titleObj = this.bar.title
							titleObj.text = '参与人数统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							canyurenshuChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								canyurenshuChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.yulemingcheng!='' && this.searchForm.yulemingcheng!=undefined){
					params['yulemingcheng'] = '%' + this.searchForm.yulemingcheng + '%'
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussyulexiangmu',query:{refid:id}});
			},
			// 删除
