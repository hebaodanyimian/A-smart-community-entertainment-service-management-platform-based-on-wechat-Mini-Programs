		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","flexWrap":"wrap","background":"#fff","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">评论内容</label>
				...
				<el-row :style='{"padding":"10px","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","color":"#fff","borderRadius":"10px","background":"#107ED6","width":"auto","fontSize":"16px","height":"40px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>

				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('discussshequhuodong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('discussshequhuodong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussshequhuodong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('discussshequhuodong','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
	...
	export default {
		...
		methods: {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussshequhuodong',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			discussistopChange(e,row){
				this.$http({
					url:'discussshequhuodong/update',
					method:'post',
					data:row
				}).then(res=>{})
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussshequhuodong/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'shequhuodong/info/' + refid,
								method: "get",
							}).then(res=>{
								if(res.data&&res.data.code==0){
									res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
									this.$http({
										url: 'shequhuodong/update',
										method: "post",
										data:res.data.data
									}).then(res1=>{
