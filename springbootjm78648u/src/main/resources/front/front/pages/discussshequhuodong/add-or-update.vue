	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussshequhuodong`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussshequhuodong`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussshequhuodong`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussshequhuodong`, this.ruleForm);
						}
						uni.setStorageSync('discussshequhuodongCleanType',true);
						if(this.cross){
