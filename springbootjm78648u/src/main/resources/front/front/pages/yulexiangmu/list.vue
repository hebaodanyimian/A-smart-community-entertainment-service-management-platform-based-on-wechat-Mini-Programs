			...
			<view :style='{"minHeight":"100%","padding":"0 20rpx","flexWrap":"wrap","background":"#E9E9E9","display":"flex","width":"100%","position":"relative","height":"auto"}'>
				...
				<view :style='{"width":"100%","flexWrap":"wrap","background":"none","display":"flex","height":"auto","order":"5"}'>
					<scroll-view scroll-x="true" class="category-one" :style='{"padding":"0","margin":"20rpx 0","whiteSpace":"nowrap","borderRadius":"20rpx","background":"none","flexDirection":"row","display":"flex","width":"100%","height":"auto"}'>
						<view :class='categoryName === item.xiangmuleixing ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.xiangmuleixing)">{{item.xiangmuleixing}}</view>
					</scroll-view>
					<!-- 样式2 -->
					<view class="list" :style='{"width":"100%","padding":"0","margin":"0","borderRadius":"20rpx","background":"none","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"0","margin":"0","height":"auto"}'>
								...
								<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx","margin":"0","overflow":"hidden","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>参与人数:{{product.canyurenshu}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yulexiangmu','修改')) || (!userid && isAuthFront('yulexiangmu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yulexiangmu','删除')) || (!userid && isAuthFront('yulexiangmu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yulexiangmu','修改')) || (!userid && isAuthFront('yulexiangmu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('yulexiangmu','删除')) || (!userid && isAuthFront('yulexiangmu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"20rpx ","color":"rgb(255, 255, 255)","outline":"none","borderRadius":"20rpx","background":"linear-gradient(45deg, #3d4e81, #5753c9, #6e7ff3)","width":"120rpx","lineHeight":"70rpx","fontSize":"28rpx","position":"relative","height":"70rpx","zIndex":"1","order":"3"}' v-if="userid && isAuth('yulexiangmu','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"20rpx ","color":"rgb(255, 255, 255)","outline":"none","borderRadius":"20rpx","background":"linear-gradient(45deg, #3d4e81, #5753c9, #6e7ff3)","width":"120rpx","lineHeight":"70rpx","fontSize":"28rpx","position":"relative","height":"70rpx","zIndex":"1","order":"3"}' v-if="!userid && isAuthFront('yulexiangmu','新增')" class="add-btn" @click="onAddTap()">新增</button>
			</view>
		</view>
	</mescroll-uni>
</template>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('xiangmuleixing', {page:1,limit:100});
            } else {
                res = await this.$api.list('xiangmuleixing', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,xiangmuleixing:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.xiangmuleixing = '%' + this.categoryName + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yulexiangmu`, params);
                } else {
                    res = await this.$api.list(`yulexiangmu`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yulexiangmu', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.categoryName!='全部'){
					searchForm.xiangmuleixing = '%' + this.categoryName + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yulexiangmu`, searchForm);
                } else {
                    res = await this.$api.list(`yulexiangmu`, searchForm);
                }
				// 如果是第一页数据置空
