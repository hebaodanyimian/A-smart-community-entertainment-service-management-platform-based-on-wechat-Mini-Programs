	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyulexiangmu`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyulexiangmu`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussyulexiangmu`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussyulexiangmu`, this.ruleForm);
						}
						uni.setStorageSync('discussyulexiangmuCleanType',true);
						if(this.cross){
