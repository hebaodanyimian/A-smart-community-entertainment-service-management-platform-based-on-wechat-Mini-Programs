		...
		<view class="container" :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#E9E9E9","height":"auto"}'>
			<view :style='{"padding":"20rpx","margin":"-80rpx 0 0","background":"none","width":"100%","position":"relative","height":"auto","zIndex":"9"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"10rpx 24rpx","flexWrap":"wrap","background":"#FFFFFF","display":"flex","gap":"20rpx","width":"100%","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","borderRadius":"8rpx","background":"linear-gradient(45deg, #3d4e81, #5753c9, #6e7ff3)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('discussyulexiangmu','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","borderRadius":"8rpx","background":"linear-gradient(45deg, #3d4e81, #5753c9, #6e7ff3)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('discussyulexiangmu','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyulexiangmuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyulexiangmuCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussyulexiangmu', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussyulexiangmu');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussyulexiangmu');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussyulexiangmu', this.id);
				let reg=new RegExp('http://localhost:8080/springbootjm78648u/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discussyulexiangmu',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussyulexiangmu', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussyulexiangmu',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussyulexiangmu/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussyulexiangmu',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussyulexiangmu',that.detail)
							setTimeout(()=>{
